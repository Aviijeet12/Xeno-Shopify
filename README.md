...
### Managing Products Per Tenant
```bash
# List products the dashboard will render for a tenant
curl -X GET "$BASE_URL/api/{tenantId}/products" -H "Authorization: Bearer <jwt>"

# Manually add a product (Shopify product id optional; autogenerated when omitted)
curl -X POST "$BASE_URL/api/{tenantId}/products" \
  -H "Authorization: Bearer <jwt>" \
  -H "Content-Type: application/json" \
  -d '{"title":"Aurora Velvet Sofa","price":1899.00}'
```
The dashboard’s Products tab now uses these APIs, so decor/tech tenants show distinct mock data after syncing, and you can seed extra catalog items directly from the UI.
